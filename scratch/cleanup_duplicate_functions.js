...
const cleanCarouselJS = `// Hero Carousel Logic (3-Slide)
...
files.forEach(file => {
  ...
  // Find where Carousel JS starts (any version of Hero Carousel Logic)
  const carouselStartIdx = content.indexOf('// Hero Carousel Logic');
  const scriptEndIdx = content.lastIndexOf('</script>');

  if (carouselStartIdx !== -1 && scriptEndIdx !== -1) {
    content = content.slice(0, carouselStartIdx) + cleanCarouselJS + '\n  ' + content.slice(scriptEndIdx);
    fs.writeFileSync(filePath, content, 'utf8');
    console.log(`Successfully cleaned up duplicate JS functions in ${file}`);
  }
});
